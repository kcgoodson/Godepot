...
namespace AC
{
	...
	public class SpeechManager : ScriptableObject
	{
		...
		public void ShowGUI ()
		{
			...
			displayForever = CustomGUILayout.ToggleLeft ("Display speech forever until user skips it?", displayForever, "AC.KickStarter.speechManager.displayForever");
			if (displayForever)
			{
				...
				allowGameplaySpeechSkipping = CustomGUILayout.ToggleLeft ("Speech during gameplay can also be skipped?", allowGameplaySpeechSkipping, "AC.KickStarter.speechManager.allowGameplaySpeechSkipping");
			}
			else
			{
			...
			if (scrollSubtitles || scrollNarration)
			{
				...
				textScrollCLip = (AudioClip) CustomGUILayout.ObjectField <AudioClip> ("Default text scroll audio:", textScrollCLip, false, "AC.KickStarter.speechManager.textScrollClip");
		...
		private List<GVar> ExtractVariables (List<GVar> vars, bool readGlobal, bool onlySeekNew)
		{
			...
			foreach (GVar var in vars)
			{
				if (var.type == VariableType.String)
				{
					...
					{
						// Assign a new ID on creation
						SpeechLine newLine = new SpeechLine (GetEmptyID (), sceneName, var.textVal, languages.Count - 1, AC_TextType.Variable);
				...
				else if (var.type == VariableType.PopUp)
				{
					...
					{
						// Assign a new ID on creation
						SpeechLine newLine = new SpeechLine (GetEmptyID (), sceneName, var.GetPopUpsString (), languages.Count - 1, AC_TextType.Variable);
				...
				for (int y = 1; y < csvOutput.GetLength (1); y++)
				{
					if (csvOutput [0,y] != null && csvOutput [0,y].Length > 0)
					{
						...
						if (int.TryParse (csvOutput [0,y], out lineID))
						{
							...
							{
								ACDebug.LogError ("Error importing translation (ID:" + csvOutput [0,y] + ", row: " + y + ") - make sure that the CSV file is delimited by a '" + CSVReader.csvDelimiter + "' character.");
							}
						}
						else
						{
		...
		private void ImportGameText ()
		{
			...
			if (File.Exists (fileName))
			{
				...
				for (int y = 1; y < csvOutput.GetLength (1); y++) // Iterate rows
				{
					if (csvOutput [0,y] != null && csvOutput [0,y].Length > 0)
					{
						lineID = -1;
						if (int.TryParse (csvOutput [0,y], out lineID))
						{
							...
							if (languages.Count > 1)
							{
								for (int i=1; i<languages.Count; i++)
								{
									...
									if (csvOutput [col, y] == null)
									{
										continue;
									}

									if (csvOutput.GetLength (0) > col)
									{
										translationText = csvOutput [col, y].Replace (CSVReader.csvTemp, CSVReader.csvComma);
										UpdateTranslation (i, lineID, owner, AddLineBreaks (translationText));
									}
									else
									{
										ACDebug.LogWarning ("Error importing file - columns do not match expected number.");
									}
								}
							}
						}
		...
		private void ExportGameText ()
		{
			...
			foreach (SpeechLine line in lines)
			{
				...
				for (int t=0; t<line.translationText.Count; t++)
				{
					if (t >= languages.Count-1) continue;

					string translationText = line.translationText[t];
